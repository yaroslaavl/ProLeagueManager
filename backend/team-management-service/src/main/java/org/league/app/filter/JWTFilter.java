...
public class JWTFilter extends OncePerRequestFilter {
    ...
    protected boolean shouldNotFilter(HttpServletRequest request) {
        ...
        return path.equals("/api/team/allTeams")
                || path.startsWith("/api/team/currentTeam/")
                || path.startsWith("/api/team/team-logo/")
                || path.equals("/api/team/get-teams-by-userId")
    ...
    protected void doFilterInternal(HttpServletRequest request,
        ...
        String email;
        log.info("Authorization header: {}", authorizationHeader, jwtToken);
        try {
            ...
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            response.getWriter().write("Invalid or expired JWT token: " + authorizationHeader);
            return;
        }
